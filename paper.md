...
With jabberwocky’s ``catch`` command, a user can provide textual data and a chosen ontology. The script will take in a JSON formatted or TXT text file and clean them. The JSON file must be provided with a parameter for the texts. Using the cleaned textual data, ``catch`` returns the key elements of the textual data which contains a class term or synonym from the ontology. Adding keywords, ``catch`` will search the ontology’s classes with the keywords and match their synonyms - providing a more precise search.

Unstructured textual data is underused. Gaining the key textual elements or values can be complicated due to limited terms being used. There are clinical terms a user can use, however synonyms expand a search horizon: exact synonyms could be other clinical terms and related synonyms (patient-preferred/natural-language/layman’s terms). To extract important text from a corpus, a user will need to use a combination of terms and synonyms for a larger output - the types of syonyons a user will need depends on their textual data, e.g. individual blogs may need natural-language synonyms. 
...
In the repository, https://github.com/sap218/jabberwocky-tests, I use an example for ``catch``. With the blog posts and ontology, the script returned 22 posts using all classes and synonyms from the ontology, as expected. Out of the 26 example blog posts, jabberwockys’ ``catch`` returned 13 posts using the keywords I provided: again as expected. Using ``grep`` and only a single word, such as “generation 1”, the output was 1, as expected. When using the ontology and the keyword, the output increases to 3. 

Using Stanford CoreNLP was a different approach: first I had to play with the ontology in order to gain each class, synonyms, and their ID codes. Stanford CoreNLP actually returned 35 posts (more than the original 26), this is because it separated each post by their full-stop (``catch`` returns the full post) and so output was larger, essentially acting as if each sentence was a separate blog post. One would have to again play with data: in order to see the output properly since Stanford CoreNLP structures the output in a particular way: again revealing the limitations in this area. Compared to jabberwocky’s ``catch``, the command simply returns the texts which contain the label/phrase for individuals to then conduct their own further research on the text itself. The ``catch`` command returns the text in the terminal which a user could use ``>`` to redirect into a file. 
